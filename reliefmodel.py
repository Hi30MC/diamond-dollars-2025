...
def write_r_scores(years: [str]) -> None:
    ...
    for year in years:
        ...
        out = pd.concat([pd.Series({stat : 1/[*sm._zstat_generic(row[stat], means[stat], stds[stat], "smaller")][1]-1 for stat in "ERA  TB/G fly_out/G walk/G".split()} | {stat : 1/[*sm._zstat_generic(row[stat], means[stat], stds[stat], "larger")][1]-1 for stat in "K/G chase%/G strike/G d2S/G".split()}).rename(i) for i, row in year_data.iterrows()], axis=1).T
